...
target_qubit_freq = 5.713443e9
qubit_freq_max = 6.409e9

path = "C:\\Users\\qcrew\\Desktop\\qcrew\\qcrew\\config\\rrfreq_vs_current\\"
filename = "20231004_095946_rr_current.npz"
...
fitted_curve = cosine_function(
    currents, fitted_amplitude, fitted_freq, fitted_phase
) + np.average(rr_freqs)
...
ax[0].scatter(currents * 1e3, rr_freqs, label="RR frequency data")
limit_current_grid = np.linspace(tunable_current_min, tunable_current_max, 1000)
ax[0].plot(
    limit_current_grid * 1e3,
    rr_freqs_interpolated(limit_current_grid),
    label="Tunability region",
)
ax[0].plot(
    currents * 1e3,
    fitted_curve,
...
ax[0].set_title(
    f"Periodicity: {1/fitted_freq*1e3:.3f}mA\nmax: {tunable_current_max*1e3:.3f}mA, min: {tunable_current_min*1e3:.3f}mA"
)
if qubit_freq_max:
    # Get ideal qubit frequency curve
    qubit_freq_fn = lambda amp: qubit_freq_max * np.sqrt(
        np.abs(np.cos(np.pi * (fitted_freq * amp + fitted_phase)))
    )
    ax[1].plot(
        currents_grid * 1e3,
        [qubit_freq_fn(amp) for amp in currents_grid],
        label="Ideal qubit frequency curve",
    )
    if target_qubit_freq and target_qubit_freq < qubit_freq_max:
        ...
        bounds = [sorted([tunable_current_min, tunable_current_max])]
        print(bounds)
        ...
        ax[1].scatter(
            ...
            target_qubit_freq,
            c="r",
            label=f"Target qubit frequency ({target_current*1e3:.3f}mA)",
            zorder=2,
        )
