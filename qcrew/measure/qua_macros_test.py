...
class ExpVariable(Variable):
    def __init__(self, 
        ...
        external_params = copy.deepcopy(kwargs)
        if value: 
            external_params.update({"length":len(value)})

        super().__init__(name=name, **external_params)
...
class SweepVariable(ExpVariable):
    def __init__(self, 
        ...
        if sweep: 
            ...
            self.stop = sweep[1]
            self.step = sweep[2]
            self.length = len(np.arange(self.start, self.stop+self.step/2, self.step))
...
def IQ_results(I_stream, Q_stream, buffer_shape:Optional[tuple]=None, tag:List[str]=["I", "Q"]) -> list:
    i = tag[0]
    q = tag[1]
    ...
    result_var_list = []
    result_var_list.append(ResultVariable(f"{i}_RAW", buffer_shape=buffer_shape, average=False, save_all=True))
    result_var_list.append(ResultVariable(f"{q}_RAW", buffer_shape=buffer_shape, average=False, save_all=True))
    result_var_list.append(ResultVariable(f"{i}_RAW_AVG", buffer_shape=buffer_shape, average=True, save_all=True))
    result_var_list.append(ResultVariable(f"{q}_RAW_AVG", buffer_shape=buffer_shape, average=True, save_all=True))
    result_var_list.append(ResultVariable(f"{i}_AVG", buffer_shape=buffer_shape, average=True, save=True))
    result_var_list.append(ResultVariable(f"{q}_AVG", buffer_shape=buffer_shape, average=True, save=True))

    return result_var_list


def ADC_results(adc_stream, input:int=1, buffer_shape:Optional[tuple]=None, tag:str="ADC") -> list:
    ...
    result_var_list = []
    result_var_list.append(ResultVariable(f"{tag}_TIMESTAMPS", buffer_shape=buffer_shape, average=False, save_all=True))
    result_var_list.append(ResultVariable(f"{tag}_RAW", buffer_shape=buffer_shape, average=False, save_all=True))

    return result_var_list

def STATE_results(state_stream,  buffer_shape:Optional[tuple]=None, tag:str="STATE") -> list:
    ...
    result_var_list = []
    result_var_list.append(ResultVariable(f"{tag}", buffer_shape=buffer_shape, average=False, save_all=True))

    return result_var_list

def Z_results(I_stream, Q_stream, buffer_shape:Optional[tuple], tag:str="Z_SQ") -> list:
    ...
    result_var_list = []
    result_var_list.append(ResultVariable(f"{tag}_RAW", buffer_shape=buffer_shape, average=False, save_all=True))
    result_var_list.append(ResultVariable(f"{tag}_RAW_AVG", buffer_shape=buffer_shape, average=True, save_all=True))
    result_var_list.append(ResultVariable(f"{tag}_AVG", buffer_shape=buffer_shape, average=True, save=True))
    return result_var_list


def qua_loop(qua_funciton:Callable,
    ...
    if len(sweep_variables) == len(declared_variables) and len(sweep_variables) != 0:
        for index, var in enumerate(sweep_variables):
            if var.value is None and var.start is not None:
                ...
                with qua.for_(swept_var, var.start, swept_var < var.stop + var.step/2, swept_var + var.step):
...
def qua_var_declare(variables:List[Union[ExpVariable,StreamVariable, SweepVariable]]) -> list:
