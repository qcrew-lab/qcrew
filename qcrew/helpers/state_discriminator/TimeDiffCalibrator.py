...
class TimeDiffCalibrator:
    def __init__(self, 
                ...
                pulse_length=None, 
                reps=None, 
                analog_input=None) -> None:
        
        self.qmm = qmm
        if  qe in config["elements"]: 
            
            self.qe = qe
            self.old_lo_freq = config["elements"][self.qe]["intermediate_frequency"]
            self.old_int_freq = config["elements"][self.qe]["intermediate_frequency"]
            
            self.int_freq = int_freq or -1e6
            self.lo_freq = self.old_lo_freq + self.old_int_freq  - self.int_freq
            
            self.pulse_length = pulse_length or 2000
            
            self.reps = reps or 1
            
            self.config = self._update_config(int_freq=self.int_freq, config=config, qe=self.qe, pulse_length=self.pulse_length)
            ...
            self.analog_input_port = self.config["elements"][self.qe]["outputs"][self.analog_input]

        else: 
            raise ValueError("The element " + qe + "does not exit in the configuration.")


    def update_lo_freq(self) -> int:
        # TODO: add the statement of setting lo frequency of lo generator (LabBrick)
        return self.new_lo_freq

    
    def rollback_lo_freq(self) -> int: 
        # TODO: add the statement of setting lo frequency of lo generator (LabBrick)
        return self.lo_freq
    ...
    def _update_config(int_freq, config, qe, pulse_length):
        ...
        config["mixers"][config["elements"][qe]["mixInputs"]["mixer"]][0]["intermediate_frequency"] = int_freq
        ...
        if analog_input == "out1":
            with program() as cal_phase:
                ...
                with for_(n, 1, n < reps+1/2, n + 1):
                    reset_frame("rr")
                    measure("time_diff_long_readout", qe, adc,
            ...
            with program() as cal_phase:
                ...
                with for_(n, 1, n < reps+1, n + 1):
                    reset_phase(qe)
                    measure("time_diff_long_readout", qe, adc_st, 
            ...
            raise ValueError("imput_port must be either 1 or 2")
    ...
    def calibrate(self, simulate=False, **execute_args)-> None:
        ...
        """
        cal_phase = self.qua_program(reps=self.reps, qe=self.qe, analog_input=self.analog_input)
        ...
        if not simulate: 
            ...
            simulation_config = SimulationConfig(duration=time,
                                    simulation_interface=LoopbackInterface(
                                    [(self.i_port[0], self.i_port[1], self.analog_input_port[0], self.analog_input_port[1]),
            ...
            # adc_avg_demod = adc_avg
            # adc_ts_demod = adc_ts
            sig_cos = adc_avg_demod * np.cos(2*np.pi * self.int_freq * 1e-9* adc_ts_demod)

            #TODO: check the sign
            sig_sin = adc_avg_demod * np.sin(2*np.pi * self.int_freq* 1e-9* adc_ts_demod)

            I_ = np.sum(sig_cos)
            Q_ = np.sum(sig_sin)

            
            time_diff_ns = np.angle((I_avg + 1j * Q_avg) / (I_ + 1j * Q_)) / 1e-9 / 2 / np.pi / np.abs(self.int_freq)
            time_diff = np.round(time_diff_ns / 4) * 4

            print("demod adc length", np.shape(adc_ts_demod))
            print("adc length", np.shape(adc_ts))
            print("I:",I_avg)
            print("Q:",Q_avg)
            print("I_:",I_)
            print("Q_:",Q_)
