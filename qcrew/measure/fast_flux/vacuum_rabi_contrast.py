...
class VacuumRabiContrast(Experiment):

    name = "vacuum_rabi_contrast"
    ...
    def QUA_play_pulse_sequence(self):
        ...
        for flux_len in self.internal_sweep:
            qua.align()
            qubit.play(self.qubit_op, ampx=0)  # play pi qubit pulse
            qua.align(flux.name, qubit.name)
            flux.play(f"sharper_square_{flux_len}ns", ampx=self.x)
...
if __name__ == "__main__":
    ...
    experiment = VacuumRabiContrast(**parameters)
