"""
Signal Hound SA124B Python API
-*- coding: utf-8 -*-

# Copyright (c) 2019 Signal Hound
# For licensing information, please see the API license in the software_licenses folder

# changes made by @atharvjoshi are signed as e.g. AJ: add module docstring
"""

# AJ: ran formatter (black)
# AJ: suppress linter warnings
# pylint: disable=invalid-name, Signal Hound's naming decision overrides pylint

# AJ: remove wildcard imports
from ctypes import CDLL, POINTER, byref, c_char_p, c_double, c_float, c_int
from pathlib import Path  # AJ

import numpy

from qcrew.helpers import logger  # AJ

# AJ: place dll in the same package as this module
...
def sa_open_device_by_serial(serial_number):
    ...
    return {"status": status, "handle": device.value}
...
def sa_close_device(device):
    return {"status": saCloseDevice(device)}


@error_check
def sa_preset(device):
    return {"status": saPreset(device)}
...
def sa_get_serial_number(device):
    ...
    return {"status": status, "serial": serial.value}
...
def sa_get_device_type(device, device_type):
    ...
    return {"status": status, "device_type": device_type.value}


@error_check
def sa_config_acquisition(device, detector, scale):
    return {"status": saConfigAcquisition(device, detector, scale)}


@error_check
def sa_config_center_span(device, center, span):
    return {"status": saConfigCenterSpan(device, c_double(center), c_double(span))}


@error_check
def sa_config_level(device, ref):
    return {"status": saConfigLevel(device, c_double(ref))}


@error_check
def sa_config_gain_atten(device, atten, gain, pre_amp):
    return {"status": saConfigGainAtten(device, atten, gain, pre_amp)}
...
def sa_config_RBW_shape(device, rbw_shape):
    return {"status": saConfigRBWShape(device, rbw_shape)}


@error_check
def sa_config_proc_units(device, units):
    return {"status": saConfigProcUnits(device, units)}


@error_check
def sa_config_IQ(device, decimation, bandwidth):
    return {"status": saConfigIQ(device, decimation, c_double(bandwidth))}
...
def sa_config_audio(
    ...
    bandwidth,
    audio_low_pass_freq,
    audio_high_pass_freq,
    fm_deemphasis,
):
    return {
        "status": saConfigAudio(
            device,
            audio_type,  # AJ: correct variable name typo
            c_double(center_freq),  # AJ: correct variable name typo
            c_double(bandwidth),
            c_double(audio_low_pass_freq),
            c_double(audio_high_pass_freq),
            c_double(fm_deemphasis),
        )
    }
...
def sa_config_real_time(device, frame_scale, frame_rate):
    return {"status": saConfigRealTime(device, c_double(frame_scale), frame_rate)}


@error_check
def sa_config_real_time_overlap(device, advance_rate):
    return {"status": saConfigRealTimeOverlap(device, c_double(advance_rate))}


@error_check
def sa_set_timebase(device, timebase):
    return {"status": saSetTimebase(device, timebase)}


@error_check
def sa_initiate(device, mode, flag):
    return {"status": saInitiate(device, mode, flag)}


@error_check
def sa_abort(device):
    return {"status": saAbort(device)}
...
def sa_query_sweep_info(device):
    ...
    bin_size = c_double(-1)
    status = saQuerySweepInfo(
        device, byref(sweep_length), byref(start_freq), byref(bin_size)
    )
    return {
        "status": status,
...
def sa_query_stream_info(device):
    ...
    status = saQueryStreamInfo(
        device, byref(return_len), byref(bandwidth), byref(samples_per_second)
    )
    return {
        "status": status,
...
def sa_query_real_time_POI(device):
    ...
    return {"status": status, "poi": poi.value}
...
def sa_query_temperature(device):
    ...
    return {"status": status, "temp": temp.value}
...
def sa_query_diagnostics(device):
    ...
    return {"status": status, "voltage": voltage.value}
...
def sa_get_sweep_32f(device):
    ...
    return {"status": status, "min": sweep_min, "max": sweep_max}
...
def sa_get_partial_sweep_32f(device):
    ...
    start = c_int(-1)
    stop = c_int(-1)
    status = saGetPartialSweep_32f(
        device, sweep_min, sweep_max, byref(start), byref(stop)
    )
    return {
        "status": status,
        "min": sweep_min,
...
def sa_attach_tg(device):
    return {"status": saAttachTg(device)}
...
def sa_is_tg_attached(device):
    ...
    return {"status": status, "attached": attached.value}


@error_check
def sa_config_tg_sweep(device, sweep_size, high_dynamic_range, passive_device):
    return {
        "status": saConfigTgSweep(
            device, sweep_size, high_dynamic_range, passive_device
        )
    }
...
def sa_store_tg_thru(device, flag):
    return {"status": saStoreTgThru(device, flag)}


@error_check
def sa_set_tg(device, frequency, amplitude):
    return {"status": saSetTg(device, c_double(frequency), c_double(amplitude))}


@error_check
def sa_set_tg_reference(device, reference):
    return {"status": saSetTgReference(device, reference)}
...
def sa_get_API_version():
    return {"api_version": saGetAPIVersion()}


def sa_get_product_ID():
    return {"product_id": saGetProductID()}


def sa_get_error_string(status):
    return {"error_string": saGetErrorString(status)}
