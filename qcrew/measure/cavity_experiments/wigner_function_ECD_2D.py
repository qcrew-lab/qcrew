...
class WignerFunction(Experiment):
    ...
    def QUA_play_pulse_sequence(self):
        ...
        # cav.play(self.cav_op, ampx = 1, phase=-0.25)
        ...
        cav.play("cc_ECD_coherent_1.5", ampx=1, phase=0)  # First positive displacement
        qua.wait(int(20 // 4), cav.name)
        cav.play("cc_ECD_coherent_1.5", ampx=-1, phase=0)  # First negative displacement
        qua.align(qubit.name, cav.name)
        qubit.play("qubit_gaussian_short_pi_pulse")  # play pi to flip qubit around X
        qua.align(cav.name, qubit.name)  # wait for qubit pulse to end
        cav.play("cc_ECD_coherent_1.5", ampx=-1, phase=0)  # Second negative displacement
        qua.wait(int(20 // 4), cav.name)
        cav.play("cc_ECD_coherent_1.5", ampx=1, phase=0)  # Second positive displacement
        qua.align(qubit.name, cav.name) 
