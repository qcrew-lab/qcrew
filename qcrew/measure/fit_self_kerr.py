...
class fit_selfKerr(object):
    ...
    def fitPfunction(self, xData, yData):
        ...
        fre, offset, scaling = optimize.curve_fit(self.Pfunction,xData,yData,p0=p0)[0]
...
filename = r"C:/Users/qcrew/Desktop/qcrew/data/somerset/20231106/211150_somerset_Self_Kerr.h5"

# 174238_somerset_Self_Kerr
...
for i in range(len(alphalist)):
    ...
    fit = fit_selfKerr(alpha = alphalist[i], omega = omega, scaling_flag = True)
    fitdata = fit.fitSfunction_alldata(xData, yData)
    frelist.append(fitdata[0][0]/(2*np.pi))
    
    plt.scatter(xData, yData + i, c = colorlist[i], s =10, label=r'$\alpha$ = {}'.format(alphalist[i]))
...
# plt.figure()
# plt.scatter(alphalist**2, frelist*1000)
# plt.scatter((alphalist**2)[[0, -1]], (frelist*1000)[[0, -1]], s = 20, label = 'removed points for fitting')
# plt.xlabel(r'$\bar{n}$')
...
params, covariance = curve_fit(linear_model, nbar_cut, selfKerr_cut)

# Extract the fitted parameters
a_fit, b_fit = params

# Generate the fitted line
y_fit = linear_model(nbar_cut, a_fit, b_fit)

# Plot the original data and the fitted line
plt.figure(figsize=(8, 6))
plt.scatter(nbar_cut, selfKerr_cut, label='Data', color='blue')
plt.plot(nbar_cut, y_fit, label='Fitted Line', color='red')
plt.xlabel('nbar_cut')
plt.ylabel('selfKerr_cut')
plt.legend()
plt.show()

# Print the fitted parameters
print('slop = {} \n offset = {}'.format(a_fit, b_fit))
print("Kerr = (kHz) =", a_fit*2 )
print("delta = offset (kHz)=", b_fit )