"""
A python class describing a ef power rabi measurement using QM.
This class serves as a QUA script generator with user-defined parameters.
"""

from typing import ClassVar

from qcrew.control import professor as prof
from qcrew.measure.experiment import Experiment
from qm import qua


# ---------------------------------- Class -------------------------------------


class QubitPopulationFastFlux(Experiment):

    name = "qubit_population_ff"

    _parameters: ClassVar[set[str]] = Experiment._parameters | {
        "qubit_ge_pi",
        "qubit_ef_pi",  # operation used for exciting the qubit
        "fit_fn",  # fit function
    }

    def __init__(
        self,
        qubit_ge_pi,
        qubit_ef_pi,
        flux_pulse,
        flux_amp,
        measure_delay,
        fit_fn="sine",
        **other_params
    ):

        self.qubit_ge_pi = qubit_ge_pi
        self.qubit_ef_pi = qubit_ef_pi
        self.fit_fn = fit_fn
        self.flux_pulse = flux_pulse
        self.flux_amp = flux_amp
        self.measure_delay = measure_delay

        super().__init__(**other_params)  # Passes other parameters to parent

    def QUA_play_pulse_sequence(self):
        """
        Defines pulse sequence to be played inside the experiment loop
        """
        qubit, qubit_ef, flux, rr = self.modes  # get the modes
        flux.play(self.flux_pulse, ampx=self.flux_amp)
        qua.wait(400, rr.name)
        qua.align(rr.name, flux.name, qubit.name)

        if 1:
            # Play RR ddrop pulse
            rr.play("ddrop_pulse")
            # Play qubit ddrop pulse
            qua.wait(int(2000 // 4), qubit.name)  # wait steady state of rr
            qubit.play("ddrop_pulse")
            qua.wait(int(2000 // 4), qubit.name)  # wait steady state of rr
        if 0:
            qua.wait(int(self.measure_delay // 4), qubit.name)

        qubit.play(self.qubit_ge_pi, ampx=self.y)
        qua.align(qubit.name, qubit_ef.name)
        qubit_ef.play(self.qubit_ef_pi, ampx=self.x)
        qua.align(qubit.name, qubit_ef.name)
        qubit.play(self.qubit_ge_pi)

        qua.align(qubit.name, rr.name)  # wait qubit pulse to end
        rr.measure((self.I, self.Q))  # measure qubit state
        if self.single_shot:
            qua.assign(
                self.state, qua.Cast.to_fixed(self.I < rr.readout_pulse.threshold)
            )
        qua.wait(int(self.wait_time // 4), rr.name)  # wait system reset

        self.QUA_stream_results()  # stream variables (I, Q, x, etc)


# -------------------------------- Execution -----------------------------------

if __name__ == "__main__":

    amp_start = -1.9
    amp_stop = 1.9
    amp_step = 0.05

    parameters = {
        "modes": ["QUBIT", "QUBIT_EF", "FLUX", "RR"],
        "reps": 100000,
        "wait_time": 100000,
        "qubit_ge_pi": "gaussian_pi",
        "qubit_ef_pi": "constant_pi",
        "flux_pulse": "predist_gaussian_pulse",
        "flux_amp": 0.7,
        "measure_delay": 8000,  # ns
        "x_sweep": (-1.8, 1.8 + 0.1 / 2, 0.1),
        "y_sweep": [0.0, 1.0],
        # "plot_quad": "I_AVG",
    }

    plot_parameters = {
        "xlabel": "Qubit pulse amplitude scaling",
    }

    experiment = QubitPopulationFastFlux(**parameters)
    experiment.setup_plot(**plot_parameters)

    prof.run(experiment)
