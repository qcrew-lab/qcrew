...
class DRAGCalibration(Experiment):
    ...
    def QUA_play_pulse_sequence(self):
        ...
        for gate_pair in self.gate_list:
            ...
            qua.wait(int(120 // 4), qubit_lk.name)

            # Play the first gate
            # The DRAG correction is scaled by self.x
            qubit_lk.play(gate1_rot, ampx=(1.0, 0.0, 0.0, self.x), phase=gate1_axis)

            # Play the second gate
            # The DRAG correction is scaled by self.x
            qubit_lk.play(gate2_rot, ampx=(1.0, 0.0, 0.0, self.x), phase=gate2_axis)

            qua.wait(int(1000 // 4), qubit_lk.name, flux.name)
           
            if 1:
                flux.play("constcos20ns_tomo_RO_tomo_new_E2pF2pG2pH2_3", ampx=-0.5705)
            ...
            qua.align(qubit_lk.name, rr.name)  # align to the next gate pair
