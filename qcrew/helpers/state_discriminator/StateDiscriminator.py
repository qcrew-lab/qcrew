...
class StateDiscriminator:
    ...
    def __init__(self,
        ...
        if self.resonator in self.config["elements"]: 
            ...
            if self.readout_pulse not in self.config["elements"][self.resonator]["operations"]:
                raise ValueError(self.readout_pulse, "dpes not exist in the operations")
            
            self.time_of_flight = self.config["elements"][self.resonator]["time_of_flight"]
        ...
        if self.qubit in self.config["elements"]: 
            ...
            if self.qubit_pulse not in self.config["elements"][self.qubit]["operations"]:
        ...
        if metadata:
            try:
                ...
                raise ValueError("metadata dictionary does not include the keys \"wait_time\" or \"reps\".")
        else:
            self.wait_time = 100
        ...
        self.path = path or 'ge_disc_params.npz'
    ...
    def _downconvert(self, qe, x, ts, simulate=False):
        
        # calculate the time difference
        if self.time_diff is None:
            td = TimeDiffCalibrator(self.qmm, self.config, qe)
            self.time_diff = td.calibrate(simulate=simulate)
            print("The time difference between the modualtion signal and the cos/sin calcualtion over the adc is ", self.time_diff, "ns")
    ...
    def _get_traces(self, qe, correction_method, I_res, Q_res, seq0, sig, use_hann_filter):
        ...
        if correction_method == 'gmm':
            ...
            gmm = mixture.GaussianMixture(n_components=self.num_of_states, covariance_type='full', tol=1e-12,
    ...
    def train(self, program=None, use_hann_filter=True, plot=False, reps=None, correction_method='robust', simulate=False, simulation_config=None, **execute_args):
        """
        ...
        results = self._execute_and_fetch(program, simulation_config = simulation_config, **execute_args)
        ...
        sig = self._downconvert(self.resonator, x_demod, ts_demod, simulate=simulate)

        traces = self._get_traces(self.resonator, correction_method, I_res, Q_res, seq0, sig, use_hann_filter)
