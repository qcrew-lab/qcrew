...
if __name__ == "__main__":
    ...
    with Stagehand() as stage:
        ...
        # Execute job
        job = stage.QM.execute(get_qua_program(rr, qubit))
        ...
        adc = np.concatenate((adc_g, adc_e), axis=0)  # This is the important stuff

        # td = TimeDiffCalibrator(qmm, config, rr.name)
        # time_diff = td.calibrate()
        # print("Time difference: ", time_diff)

        sd = StateDiscriminator(
            resonator=rr.name, config=stage.QM.get_config(), time_diff=None
        )
